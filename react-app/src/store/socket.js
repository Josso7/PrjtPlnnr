...
export const getRooms = (projectId) => async (dispatch) => {
    const res = await fetch(`/api/groups/${projectId}/rooms`);
    if (res.ok) {
...
export const getRoom = (channelId) => async (dispatch) => {
    const res = await fetch(`/api/rooms/${channelId}`);
    if (res.ok) {
        const data = await res.json();
        if (data.errors) {
            return;
...
export const createRoom = (channel_name, group_id) => async (dispatch) => {
